...
class TemplateMatch:
    ...
    def log2Dsearch(self, frame, pos_x, pos_y):
        ...
        while True:
            ...
            if d <= 1:
                break
            for i in range(pos_x-p, pos_x+p+1, d):
                for j in range(pos_y-p, pos_y+p+1, d):
                    if 0 <= i < frame_height and 0 <= j < frame_width:
                        if filtered[i][j] > cur_max:
                            pos_x = i
                            pos_y = j
...
for p in all_p:
    for method in methods:
        ...
        while cap.isOpened():
            ...
            cv2.rectangle(outframe, (start_y, start_x), (end_y, end_x), (255, 0, 0), 2)
        ...
        print("method: ",method," avererage num of search: ", total_search/id, " for p=", p)